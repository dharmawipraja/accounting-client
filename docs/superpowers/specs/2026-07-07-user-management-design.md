...
1. **Admin Users page** (`/users`, ADMIN-only): list, create, edit, deactivate/activate, reset password.
...
| DELETE | `/users/{id}` | — | `204` | `404`, `422` — **not exposed in the client for now** (see Out of scope) |
...
- **Deactivate/Activate** — `PATCH { isActive }`, reversible. A deactivate uses a `ConfirmDialog`; activate is direct. Hard delete (`DELETE /users/{id}`) is intentionally **not** exposed for now (see Out of scope) — deactivation is the removal path.
...
- **Self-lockout prevention:** in Edit, disable changing *your own* role and deactivating *yourself*; hide the deactivate row action on your own row. Still surface the API's `422` (e.g. demoting or deactivating the last admin) gracefully via toast — never assume the client guard is exhaustive.
...
- `UsersPage`: list renders, search, pagination; create → temp-password reveal; edit; reset → reveal; deactivate/activate; forbidden for non-admin; self-lockout guards.
...
- **Hard delete of users** (`DELETE /v1/users/{id}`) — the endpoint exists but is not surfaced for now; deactivation (`isActive:false`) is the removal path. Can be added later if needed.
